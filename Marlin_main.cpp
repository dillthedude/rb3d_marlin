 ...
 * M703 - Continuous Purge
 * M704 - Set probe to optimal height for calibration
 * M705 - Set probe back to original position after calibration
...
inline void gcode_G28(const bool always_home_all) {
  ...
  endstops.enable(true); // Enable endstops for next homing move
	...
	#if ENABLED(I2C_POSITION_ENCODERS)
      if (homeX || home_all){
        I2CPEM.homing(X_AXIS);
      }
      if (homeY || home_all){
        I2CPEM.homing(Y_AXIS);
      }
      if (homeZ || home_all){
        I2CPEM.homing(Z_AXIS);
      }    
    #endif
  ...
  static bool ensure_safe_temperature(const AdvancedPauseMode mode=ADVANCED_PAUSE_MODE_PAUSE_PRINT) {
	...
	*/
	static bool load_filament(const float &slow_load_length=0, const float &fast_load_length=0, const float &purge_length=0, const int8_t max_beep_count=0,
							const bool show_lcd=false, const bool pause_for_user=true,
	...
	* Continuous Purge
	**/
	void cont_purge(const bool show_lcd = false, const float purge_length = 0, const AdvancedPauseMode mode = ADVANCED_PAUSE_MODE_PAUSE_PRINT)
	{
		do 
		{
			if (purge_length > 0) 
	...
	*/
	static bool unload_filament(const float &unload_length, const bool show_lcd=false, const AdvancedPauseMode mode=ADVANCED_PAUSE_MODE_PAUSE_PRINT) 
	{
		if (!ensure_safe_temperature(mode)) {
		  #if ENABLED(ULTIPANEL)
			if (show_lcd) // Show status screen
...
inline void gcode_M502() {
  ...
  inline void gcode_M852() {
	...
	**/
	inline void gcode_M600() 
	{
		...
		// Unload filament
		const float unload_length = -ABS(parser.seen('U') ? parser.value_axis_units(E_AXIS) : filament_change_unload_length[active_extruder]);
  ...
  inline void gcode_M701() 
  {
    ...
    // Lift Z axis
    if (park_point.z > 0)
		do_blocking_move_to_z(FILAMENT_Z_LIFT_LENGTH, NOZZLE_PARK_Z_FEEDRATE+5);
    ...
    load_filament(slow_load_length, fast_load_length, ADVANCED_PAUSE_PURGE_LENGTH, FILAMENT_CHANGE_ALERT_BEEPS,
                  true, true/*thermalManager.wait_for_heating(target_extruder)*/, ADVANCED_PAUSE_MODE_LOAD_FILAMENT);

    // Restore Z axis
    if (park_point.z > 0)
		do_blocking_move_to_z(0, NOZZLE_PARK_Z_FEEDRATE+5);
	...
	**/
	inline void gcode_M702() 
	{
		...
		// Unload filament
			...
			{
				// Unload length
				const float unload_length = -ABS(parser.seen('U') ? parser.value_axis_units(E_AXIS) :FILAMENT_CHANGE_UNLOAD_LENGTH);
				unload_filament(FILAMENT_CHANGE_UNLOAD_LENGTH, true, ADVANCED_PAUSE_MODE_UNLOAD_FILAMENT);
			}
	...
	inline void gcode_M703()
	{
		point_t park_point = NOZZLE_PARK_POINT;
			...
			if (axis_unhomed_error()) park_point.z = 0;
		#endif
			if (get_target_extruder_from_command(703)) return;
			// Z axis lift
			if (parser.seenval('Z')) park_point.z = parser.linearval('Z');
		...
		// Continuous Purge filament
		cont_purge(true, ADVANCED_PAUSE_PURGE_LENGTH);
	...
	inline void gcode_M704()
	{
		...
		do_blocking_move_to_z(FILAMENT_Z_LIFT_LENGTH);
		do_blocking_move_to_xy(X_CENTER, Y_CENTER/2);
		

		#if ENABLED(LCD_BED_LEVELING)
			lcd_wait_for_move = false;							//<- compilation issue number one
		#endif
  
	}
	inline void gcode_M705()
	{
		...
		#if ENABLED(LCD_BED_LEVELING)
			lcd_wait_for_move = false;							//<- compilation issue number two
		#endif
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 502: gcode_M502(); break;                              // M502: Revert Settings to defaults
		...
		case 703: gcode_M703(); break;							  // M703: Continuous Purge
		case 704: gcode_M704(); break;							  // M704: Lift to Optimal Z
		case 705: gcode_M705(); break;							  // M705: Restore Z
      #endif
...
void setup() {
  ...
  if (mcu &  8) SERIAL_ECHOLNPGM(MSG_INFO_SCREENDOG_RESET);
