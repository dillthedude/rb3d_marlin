...
uint16_t max_display_update_time = 0;
	...
	static void lcd_filament_menu(); //JDK added
	...
	*
	* Examples:
	*   MENU_ITEM(back, MSG_INFO_SCREEN, 0 [dummy parameter] )
	*   or
	*   MENU_BACK(MSG_INFO_SCREEN)
	*     lcd_implementation_drawmenu_back(sel, row, PSTR(MSG_INFO_SCREEN))
	*     menu_action_back()
	*
	...
	*/
	...
	int8_t manual_move_axis_test = (int8_t)NO_AXIS;
	#if EXTRUDERS > 1
	...
	*/
	...
	if (currentScreen != screen) 
	{
		...
		new_z_fade_height = planner.z_fade_height;
			...
			{
				if (currentScreen == lcd_status_screen) { doubleclick_expire_ms = millis() + DOUBLECLICK_MAX_INTERVAL; }
			}
...
void lcd_reset_status() 
{
	...
	const char *msg;
	if (print_job_timer.isPaused()) { msg = paused; }
	#if ENABLED(SDSUPPORT)
		else if (card.sdprinting)
			return lcd_setstatus(card.longFilename[0] ? card.longFilename : card.filename, true);
	#endif
	else if (print_job_timer.isRunning()) { msg = printing; }
	else { msg = welcome; }
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	{
      #if ENABLED(PARK_HEAD_ON_PAUSE)
            // XY Home before continue if no skipsense //JDK beta
        #if ENABLED(RB_SKIP_SENSE) //JDK beta
        #else
          enqueue_and_echo_commands_P(PSTR("G28 X Y")); //JDK beta
        #endif //JDK beta
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		*/
		...
		{
			START_MENU();
			MENU_ITEM(function, MSG_INFO_SCREEN, lcd_return_to_status);
				...
				{
					MENU_ITEM(submenu, MSG_TASKS, lcd_tasks_menu);
			...
			**/
			...
			//---------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				**/
				...
				#if HAS_TEMP_HOTEND
					#if HAS_HEATED_BED
						void lcd_preheat_n220_b60() {_lcd_preheat(0, PREHEAT_220_TEMP_HOTEND, PREHEAT_60_TEMP_BED, lcd_preheat_fan_speed[0]);}
						void lcd_preheat_n225_b60() { _lcd_preheat(0, PREHEAT_225_TEMP_HOTEND, PREHEAT_60_TEMP_BED, lcd_preheat_fan_speed[0]); }
						void lcd_preheat_n235_b100() {_lcd_preheat(0, PREHEAT_235_TEMP_HOTEND, PREHEAT_100_TEMP_BED, lcd_preheat_fan_speed[1]);}
						void lcd_preheat_custom_nozzle() { _lcd_preheat(0, thermalManager.target_temperature[0], -1, lcd_preheat_fan_speed[1]); }
						void lcd_preheat_custom_bed() { _lcd_preheat(0, -1, thermalManager.target_temperature_bed, lcd_preheat_fan_speed[1]); }
						void lcd_preheat_custom_both() { _lcd_preheat(0, thermalManager.target_temperature[0], thermalManager.target_temperature_bed, lcd_preheat_fan_speed[1]); }
					#endif
				#endif

				void lcd_nozzle_custom_menu()
				{
					START_MENU();
					MENU_BACK(MSG_PREHEAT_CUSTOM);
					MENU_ITEM_EDIT(int3, MSG_NOZZLE, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
					MENU_ITEM(function, MSG_PREHEAT, lcd_preheat_custom_nozzle);

					END_MENU();
				}

				void lcd_bed_custom_menu()
				{
					START_MENU();
					MENU_BACK(MSG_PREHEAT_CUSTOM);
					MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_BED, &thermalManager.target_temperature_bed, 0, BED_MAXTEMP - 15, watch_temp_callback_bed);
					MENU_ITEM(function, MSG_PREHEAT, lcd_preheat_custom_bed);

					END_MENU();
				}

				void lcd_both_custom_menu()
				{
					START_MENU();
					MENU_BACK(MSG_PREHEAT_CUSTOM);
					MENU_ITEM_EDIT(int3, MSG_NOZZLE, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
				...
				void lcd_preheat_custom_menu() 
				{
					START_MENU();
					MENU_BACK(MSG_PREHEAT);
					MENU_ITEM(submenu, MSG_PREHEAT_NOZZLE, lcd_nozzle_custom_menu);
					MENU_ITEM(submenu, MSG_PREHEAT_BED   , lcd_bed_custom_menu);
					MENU_ITEM(submenu, MSG_PREHEAT_BOTH  , lcd_both_custom_menu);
					END_MENU();
				}
				...
				**/
					...
					void lcd_preheat_menu() 
					{
						...
						MENU_ITEM(submenu, MSG_TASKS, lcd_tasks_menu);
							...
							#if HAS_HEATED_BED
								MENU_ITEM(function, MSG_PREHEAT_220_60, lcd_preheat_n220_b60);
								MENU_ITEM(function, MSG_PREHEAT_225_60, lcd_preheat_n225_b60);
								MENU_ITEM(function, MSG_PREHEAT_235_100, lcd_preheat_n235_b100);
								MENU_ITEM(submenu, MSG_PREHEAT_CUSTOM, lcd_preheat_custom_menu);
							#endif
		...
		//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			**/
			...
			//---------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				/**
				 *
				 * "Manual Level Bed" inner functions 
				 *
				 **/
					...
					*/
				...
				#if ENABLED(LCD_BED_LEVELING) && (ENABLED(PROBE_MANUALLY) || ENABLED(MESH_BED_LEVELING) || ENABLED(AUTO_BED_LEVELING_BILINEAR))
					...
					void _lcd_level_goto_previous_point();
					void _lcd_level_goto_center();
					void _lcd_level_bed_finish();
					void _lcd_level_goto_previous_point_from_center();
    

					/**
					- Called from either the functions "_lcd_level_bed_display_(next/previous)_point"
					- Creates Submenu for "Continue", "Go Back", or "Finish" to get to desired probe location
					- Called the functions "_lcd_level_goto_(next_point/previous_point/finish)" 
					**/
					void _lcd_level_bed_create_submenu() 
					{
						ENCODER_DIRECTION_NORMAL();
							if (Gone_To_Center == true)
							{
								START_MENU();
								MENU_ITEM(function, MSG_FORWARD, _lcd_level_goto_next_point);
								MENU_ITEM(function, MSG_BACK, manual_probe_index++ ? _lcd_level_goto_previous_point_from_center: NULL);
								MENU_ITEM(function, MSG_FINISH,  _lcd_level_bed_finish);
								END_MENU();
							}
							else
							{
								START_MENU();
								MENU_ITEM(function, MSG_FORWARD, _lcd_level_goto_next_point);
								MENU_ITEM(function, MSG_BACK, _lcd_level_goto_previous_point);
								MENU_ITEM(function, MSG_CENTER, _lcd_level_goto_center);
								MENU_ITEM(function, MSG_FINISH, _lcd_level_bed_finish);
								END_MENU();
							}
						return;
					...
					/**
					- Called from the function "_lcd_level_goto_center()"
					- Displays "Moving to Center" while waiting for move to finish
					- Calls the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					**/
					void _lcd_level_bed_display_center()
					{
						Gone_To_Center = true;
						if (lcdDrawUpdate)
						{
							lcd_implementation_drawedit(PSTR(MSG_LEVEL_BED_CENTER), NULL);
						}
						lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
						if (!lcd_wait_for_move)
						{
							lcd_goto_screen(_lcd_level_bed_create_submenu);
						}
					}
	 
	 
					/**
					- Called from the function "_lcd_level_goto_next_point()"
					- Displays "Next point: i / Max Points" while waiting for move to finish
					- Calls the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					**/
					void _lcd_level_bed_display_next_point() 
					{
						Gone_To_Center = false;
						if (lcdDrawUpdate) 
						{
							char msg[10];
							if (manual_probe_index == total_probe_points+1) { manual_probe_index = 1; }
							sprintf_P(msg, PSTR("%i / %u"), (int)(manual_probe_index), total_probe_points);
					...
					/**
					- Called from the function "_lcd_level_goto_previous_point()"
					- Displays "Previous point: i / Max Points" while waiting for move to finish
					- Calls the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					**/
					void _lcd_level_bed_display_previous_point() 
					{
						Gone_To_Center = false;
						if (lcdDrawUpdate) 
						{
							char msg[10];
							if (manual_probe_index == 0) {manual_probe_index = 4;}
							sprintf_P(msg, PSTR("%i / %u"), (int)(manual_probe_index), total_probe_points);
							lcd_implementation_drawedit(PSTR(MSG_LEVEL_BED_PREVIOUS_POINT), msg);
						}
					  lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
					...
					/**
					- Called from the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					- Initiates a move to the center
					- Calls the function "_lcd_level_bed_display_center()" which displays "Moving to Center"
					**/
					void _lcd_level_goto_center()
					{
					...
					/**
					- Called from the function "_lcd_level_bed_display_homing_start()" for starting or called from the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					...
					**/
					...
					/**
					 - Called from the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					 - Initiates a move to the previous point
					 - Calls the function "_lcd_level_bed_display_previous_point()" which displays "Previous Point" 
					 **/
					 void _lcd_level_goto_previous_point() 
					 {
					 ...
					 /**
					 - Called from the function "_lcd_level_bed_create_submenu()" which creates a submenu for the bed leveling procedure
					 - Initiates a move to the previous point specifically from the center
					 - Calls the function "_lcd_level_bed_display_previous_point()" which displays "Previous Point" 
					 **/
					void _lcd_level_goto_previous_point_from_center() 
					{
					...
					/**
					- Called from "_lcd_level_bed_homing_finish()"
					- Displays the message "Homing to Finish"
					- Calls the function "lcd_goto_previous_menu_no_defer()" to return to "Tasks" menu
					**/
					void _lcd_level_bed_display_homing_finish()
					{
						if (lcdDrawUpdate)
						{
							lcd_implementation_drawedit(PSTR(MSG_LEVEL_BED_HOMING_FINISH), NULL);
							lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
						}
					...
					/**
					- Called from the submenu found when bed leveling in the function "_lcd_level_bed_create_submenu()"
					- Moves the probe to the home position and gives a beep for feedback
					- Calls the function "_lcd_level_bed_display_homing_finish()" that displays "Homing to Finish"
					**/ 
					void _lcd_level_bed_finish() 
					{
						defer_return_to_status = true;
						...
						lcd_goto_screen(_lcd_level_bed_display_homing_finish);
					}
		

					/**
					- Display "Click to Begin", and waits for a click and proceeds to the first point
					**/
					...
					/**
					- Called from the function "_lcd_level_bed_start()" that is called with the option "Level Bed", within "Bed Leveling", within "Tasks"
					- Displays "Homing XYZ" - Waits for homing to finish
					- Calls the function "_lcd_level_goto_next_point()" and initiates the bed leveling procedure
					**/
					void _lcd_level_bed_display_homing_start() 
					{
					...
					#endif
					/**
					- Called from the MENU "_lcd_manual_bed_leveing()"  from the Tasks Menu
					- Starts the Bed Leveing Procedure if preheated
					- Calls the function "_lcd_level_goto_next_point()" and initiates the bed leveling procedure
					**/
					void _lcd_level_bed_start_exception() 
					{
						defer_return_to_status = true;
						axis_homed[X_AXIS] = axis_homed[Y_AXIS] = axis_homed[Z_AXIS] = false;
						lcd_goto_screen(_lcd_level_bed_display_homing_start);
						enqueue_and_echo_commands_P(PSTR("G28"));
					}

					/**
					- Called from the MENU "_lcd_manual_bed_leveing()"  from the Tasks Menu
					- Starts the Bed Leveing Procedure and doesnt allow if not preheated
					- Calls the function "_lcd_level_goto_next_point()" and initiates the bed leveling procedure
					**/
					void _lcd_level_bed_start() 
					{
						if (thermalManager.targetTooColdToExtrude(active_extruder))
						{
							START_MENU();
							STATIC_ITEM(MSG_NO_PREHEAT,  true, false);
							STATIC_ITEM(MSG_CHOOSE_OPTION, true, false);
							MENU_ITEM(submenu, MSG_PREHEAT, lcd_preheat_menu);
							MENU_ITEM(function, MSG_MANUAL_LEVEL_BED, _lcd_level_bed_start_exception);
							MENU_ITEM(submenu, MSG_BACK, lcd_tasks_menu);
							END_MENU();
						}
						else 
						{
			...
			//---------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				/**
				*
				* Auto Bed Leveling Inner Functions
				*
				*/
				...
				void _lcd_ubl_validate_mesh() 
				{
				  ...
				  MENU_BACK(MSG_UBL_TOOLS);
				  #if HAS_HEATED_BED
					MENU_ITEM(gcode, MSG_UBL_VALIDATE_PLA_MESH, PSTR("G28\nG26 C B" STRINGIFY(PREHEAT_1_TEMP_BED) " H" STRINGIFY(PREHEAT_220_TEMP_HOTEND) " P"));
					MENU_ITEM(gcode, MSG_UBL_VALIDATE_ABS_MESH, PSTR("G28\nG26 C B" STRINGIFY(PREHEAT_2_TEMP_BED) " H" STRINGIFY(PREHEAT_235_TEMP_HOTEND) " P"));
				  #else
					MENU_ITEM(gcode, MSG_UBL_VALIDATE_PLA_MESH, PSTR("G28\nG26 C B0 H" STRINGIFY(PREHEAT_220_TEMP_HOTEND) " P"));
					MENU_ITEM(gcode, MSG_UBL_VALIDATE_ABS_MESH, PSTR("G28\nG26 C B0 H" STRINGIFY(PREHEAT_235_TEMP_HOTEND) " P"));
				  #endif
				...
				void _lcd_ubl_build_mesh() 
				{
				  ...
				  MENU_BACK(MSG_UBL_TOOLS);
					...
					MENU_ITEM(gcode, MSG_UBL_BUILD_PLA_MESH, PSTR(
					  ...
					  "M109 S" STRINGIFY(PREHEAT_220_TEMP_HOTEND) "\n"
					  "G29 P1\n"
					  "M104 S0\n"
					  "M140 S0"
					));
					MENU_ITEM(gcode, MSG_UBL_BUILD_ABS_MESH, PSTR(
					  ...
					  "M109 S" STRINGIFY(PREHEAT_235_TEMP_HOTEND) "\n"
					  "G29 P1\n"
					  "M104 S0\n"
					  "M140 S0"
					));
			...
			//---------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				/**
				* Level Bed Corners Setup and Menu
				**/
			...
			//---------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				/**
				*
				* "Tasks" > "Bed Leveling" Submenu Setup
				*
				**/
					...
					**/
					void lcd_bed_leveling_menu() 
					{
						...
						//#else
							...
							MENU_ITEM(gcode, MSG_AUTO_LEVEL_BED, is_homed ? PSTR("G29") : PSTR("G28\nG29"));
						#endif
						...
						#endif
						END_MENU();
					}  // End of Bed Leving Menu Setup

				#endif // LCD_BED_LEVELING MENU
				...
				*/
				inline void manage_manual_move() 
				{
					...
					if (manual_move_axis != (int8_t)NO_AXIS && ELAPSED(millis(), manual_move_start_time) && !planner.is_full()) 
					{
						...
						feedrate_mm_s = MMM_TO_MMS(manual_feedrate_mm_m[manual_move_axis_test]);
						...
						destination[manual_move_axis_test] += manual_move_offset;

						// Reset for the next move
						manual_move_offset = 0.0;
						manual_move_axis_test = (int8_t)NO_AXIS;
						...
						tasks_move_to_destination(); // will call set_current_from_destination()
						...
						{
							planner.buffer_line_kinematic(current_position, MMM_TO_MMS(temp_cal_feedrate), manual_move_e_index);
							manual_move_axis_test = (int8_t)NO_AXIS;
						}
						else
						{
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				**/
				void lcd_move_menu() 
				{
					...
					if (_MOVE_XYZ_ALLOWED) 
					{
					...
					#elif ENABLED(DUAL_X_CARRIAGE) || ENABLED(RB_DUAL)
						...
						MENU_ITEM(submenu, MSG_MOVE_E, lcd_move_get_e_amount); //Moves Extruder
						#if E_MANUAL > 1
			...
			**/
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				**/
				...
				void _change_filament_temp(const uint8_t index) 
				{
					...
					thermalManager.setTargetHotend(index == 1 ? PREHEAT_220_TEMP_HOTEND : PREHEAT_235_TEMP_HOTEND, _change_filament_temp_extruder);
				...
				/**
				* Calibrate Filament Temp
				**/

				float move_menu_scale_test;
				
				float filament_mm_to_encoder_mm = .048125;

				screenFunc_t _manual_move_func_ptr_test;
				...
				void lcd_move_menu_250mm_test() { _goto_manual_move_test(250.0 * filament_mm_to_encoder_mm); }
				void lcd_move_menu_500mm_test()  { _goto_manual_move_test(500.0 * filament_mm_to_encoder_mm); }

				void lcd_cal_finish_display()
				{
					if (lcdDrawUpdate)
					{

						lcd_implementation_drawedit(PSTR(MSG_MOVING_BACK), NULL);
					}
					lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;

					if (axis_homed[Z_AXIS] && axis_homed[X_AXIS] && axis_homed[Y_AXIS])
					{
						lcd_completion_feedback();
				...
				void lcd_cal_finish()
				{
					lcd_goto_screen(lcd_cal_finish_display);
					enqueue_and_echo_commands_P(PSTR("G28"));
					//lcd_wait_for_move = true;
				}

				void _lcd_move_distance_menu_test() 
				{
					char msg[10];
					...
					STATIC_ITEM("",true); // invisible menu item
					#if HOTENDS == 1
						MENU_ITEM_EDIT(int3, MSG_NOZZLE, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
					#else // HOTENDS > 1
						MENU_ITEM_EDIT(int3, MSG_NOZZLE MSG_N1, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
						MENU_ITEM_EDIT(int3, MSG_NOZZLE MSG_N2, &thermalManager.target_temperature[1], 0, HEATER_0_MAXTEMP - 15);
						#if HOTENDS > 2
						  MENU_ITEM_EDIT(int3, MSG_NOZZLE MSG_N3, &thermalManager.target_temperature[2], 0, HEATER_0_MAXTEMP - 15);
							#if HOTENDS > 3
							  MENU_ITEM_EDIT(int3, MSG_NOZZLE MSG_N4, &thermalManager.target_temperature[3], 0, HEATER_0_MAXTEMP - 15);
								#if HOTENDS > 4
								  MENU_ITEM_EDIT(int3, MSG_NOZZLE MSG_N5, &thermalManager.target_temperature[4], 0, HEATER_0_MAXTEMP - 15);
								#endif // HOTENDS > 4
							#endif // HOTENDS > 3
						#endif // HOTENDS > 2
					#endif // HOTENDS > 1
					...
					MENU_ITEM(function, MSG_EXTRUDE_500MM, lcd_move_menu_500mm_test);
					MENU_ITEM(submenu, MSG_MOVE_AXIS, lcd_move_menu);
					MENU_ITEM(function, MSG_FINISH, lcd_cal_finish);
					END_MENU();
				}
					
				void _lcd_display_moving()
				{
					if (lcdDrawUpdate)
					{
						lcd_implementation_drawedit(PSTR(MSG_MOVING), NULL);
					}
					lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
					
					if (!lcd_wait_for_move)
					{ 
						lcd_goto_screen(_lcd_move_distance_menu_test); 
					}
				}

				void lcd_move_to_good_z()
				{
					defer_return_to_status = true;
					lcd_goto_screen(_lcd_display_moving);
					enqueue_and_echo_commands_P(PSTR("M704"));
					//lcd_wait_for_move = true;
				}

				void _lcd_display_homing()
				{
					if (lcdDrawUpdate) 
					{
						lcd_implementation_drawedit(PSTR(MSG_HOMING_Z), NULL);	
					}
					lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
				...
				void lcd_move_get_e_amount_test()        
				{ 
					defer_return_to_status = true;
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------------
						...
						**/
						void lcd_change_filament_menu() 
						{
							...
							// Change filament
								...
								if (thermalManager.targetTooColdToExtrude(0)) { MENU_ITEM_P(submenu, msg0, lcd_temp_menu_e0_filament_change); }
								else { MENU_ITEM_P(gcode, msg0, PSTR("M600 B0 T0")); }

								if (thermalManager.targetTooColdToExtrude(1)) { MENU_ITEM_P(submenu, msg1, lcd_temp_menu_e1_filament_change); }
								else { MENU_ITEM_P(gcode, msg1, PSTR("M600 B0 T1")); }

								#if E_STEPPERS > 2
									PGM_P msg2 = PSTR(MSG_FILAMENTCHANGE " " MSG_E3);
									if (thermalManager.targetTooColdToExtrude(2)) { MENU_ITEM_P(submenu, msg2, lcd_temp_menu_e2_filament_change); } 
									else { MENU_ITEM_P(gcode, msg2, PSTR("M600 B0 T2")); }

								#if E_STEPPERS > 3
									PGM_P msg3 = PSTR(MSG_FILAMENTCHANGE " " MSG_E4);
									if (thermalManager.targetTooColdToExtrude(3)) { MENU_ITEM_P(submenu, msg3, lcd_temp_menu_e3_filament_change); }
									else { MENU_ITEM_P(gcode, msg3, PSTR("M600 B0 T3")); }

								#if E_STEPPERS > 4
									PGM_P msg4 = PSTR(MSG_FILAMENTCHANGE " " MSG_E5);
									if (thermalManager.targetTooColdToExtrude(4)) { MENU_ITEM_P(submenu, msg4, lcd_temp_menu_e4_filament_change); }
									else { MENU_ITEM_P(gcode, msg4, PSTR("M600 B0 T4")); }
								...
								if (!planner.movesplanned() && !IS_SD_FILE_OPEN) 
								{
									...
									// Unload filament
										...
										if (thermalManager.targetHotEnoughToExtrude(active_extruder))
										{
											MENU_ITEM(gcode, MSG_FILAMENTPURGE, PSTR("M703"));
										}
				...
				void lcd_advanced_pause_option_menu() 
				{
					...
					MENU_ITEM(function, MSG_FILAMENT_CHANGE_OPTION_FINISH, lcd_advanced_pause_resume_print);
					END_MENU();
				}
		...
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			**/
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				/**
				*
				* "Delta Calibration" Inner Functions
				*
				**/
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				/**
				*
				* "Delta Calibration" Submenu Setup
				*
				**/
		...
		**/
			...
			**/
			void lcd_tasks_menu() 
			{
				...
				//  Homing
				//
				MENU_ITEM(submenu, MSG_HOMING, lcd_homing_menu);
				...
				//
					...
					if (!IS_SD_FILE_OPEN) 
					{
							...
							if (thermalManager.targetHotEnoughToExtrude(active_extruder))
								...
								MENU_ITEM(submenu, MSG_FILAMENTCHANGE, lcd_temp_menu_e0_filament_change);
						#else
							MENU_ITEM(submenu, MSG_FILAMENT, lcd_change_filament_menu);
						#endif
					}
				#endif // ADVANCED_PAUSE_FEATURE
...
void lcd_update() 
{
	...
	) 
	{
		...
		if (
			#if ENABLED(ULTIPANEL)
			currentScreen == _lcd_move_distance_menu_test &&
			#endif
			!lcd_status_update_delay--
		) 
		{
