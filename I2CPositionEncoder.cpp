...
//SERIAL_ECHOLNPGM(" requesting position");
    ...
    if (!passes_test(false)) { //check encoder data is good
      //if passes test is false because of a missed communication then don't treat it like bad strength
      if (H > I2CPE_MAG_SIG_BAD) return; 
      
      if (trusted && millis() < lastErrorTime + 200) { //commented out as part of the note below
        trusted = false;
//        lcd_setstatusPGM(PSTR("SS Fault"),3);
    ...
    if (!trusted) {
      ...
      /*TESTING*/
        ...
        if (millis() - lastErrorTime > I2CPE_TIME_TRUSTED) {
          errorCount++;
          if (errorCount > 10){
            set_active(false); //disable this encoder for having too many faults
            SERIAL_ECHOPGM("Poor magnetic strength detected too often on ");
            SERIAL_ECHO(axis_codes[encoderAxis]);
            SERIAL_ECHOLNPGM(" axis encoder. Disabling. Please verify that encoder is properly installed.");
            return;
          }
          trusted = true;
...
//SERIAL_ECHO(axis_codes[encoderAxis]);
//SERIAL_ECHOLNPGM(" set up and enabled.");
...
//SERIAL_ECHO(axis_codes[encoderAxis]);
//SERIAL_ECHOLNPGM(" average calculated");
      ...
      //SERIAL_ECHOLN(error);
        ...
        if (errIdx == 0) {
          ...
          // initCorrect allows for one first correct regardless of threshold value. 
          // This makes sure there is no lingering error ready to cause a shift randomly during the print
          if ((ABS(error) > threshold * planner.axis_steps_per_mm[encoderAxis] || initCorrect) &&
              diffSum < 10 * (I2CPE_ERR_ARRAY_SIZE - 1) /*&& ABS(error) < 2000*/) { // Check for persistent error (skip)
//SERIAL_ECHO(axis_codes[encoderAxis]);
//SERIAL_ECHOLNPGM(" error identified");
            ...
            if (errPrstIdx >= I2CPE_ERR_PRST_ARRAY_SIZE) {
              ...
              errPrstIdx = 0;
              if (initCorrect) { //once initial correct has been completed the threshold is reinstated
                initCorrect = false;
              } 
              else {//lcd_setstatusPGM(PSTR("SS Corrected"),3); //no lcd report on initial correction
              }
            }
          }
          else
...
//SERIAL_ECHO(axis_codes[encoderAxis]);
//SERIAL_ECHOLNPGM(" update complete");
  }
  ...
  void I2CPositionEncoder::set_homed() {
    if (active) {
      ...
      errIdx = 0;
      errPrstIdx = 0;
      thermalManager.babystepsTodo[encoderAxis] = 0;
  ...
  void I2CPositionEncoder::set_homing() {
    if (active) {
      ...
      #ifdef I2CPE_DEBUG
        SERIAL_ECHO(axis_codes[encoderAxis]);
        SERIAL_ECHOLNPGM(" axis homing. Correction disabled.");
      #endif
    }
  }
...
//SERIAL_ECHO(axis_codes[encoderAxis]);
//SERIAL_ECHOLNPGM(" requestingFrom wire");

    if (Wire.requestFrom((int)i2cAddress, 3) != 3) {
      //houston, we have a problem...
      H = I2CPE_MAG_SIG_NF;
SERIAL_ECHO(axis_codes[encoderAxis]);
SERIAL_ECHOLN(" axis incomplete encoder response.");
      while (Wire.available()) Wire.read();  //Clear buffer and try again? JDK
SERIAL_ECHO(axis_codes[encoderAxis]);
SERIAL_ECHOLNPGM(" buffer cleared");
      Wire.begin();
SERIAL_ECHOLNPGM("i2c reinitalized");
      if (Wire.requestFrom((int)i2cAddress, 3) != 3) {
SERIAL_ECHO(axis_codes[encoderAxis]);
SERIAL_ECHOLN(" axis second attempt failed.");
  
        return 0;
      }
    }
//SERIAL_ECHO(axis_codes[encoderAxis]);
//SERIAL_ECHOLNPGM(" reading wire");

    while (Wire.available() && index < sizeof(encoderCount.bval))
      encoderCount.bval[index++] = (uint8_t)Wire.read();

    if (Wire.available()) {
      //Corrupted data suspected
      H = I2CPE_MAG_SIG_NF;
      SERIAL_ECHO(axis_codes[encoderAxis]);
      SERIAL_ECHOLN(" encoder possible corrupted data. Index overrun.");
...
//    SERIAL_ECHO(axis_codes[encoderAxis]);
//    SERIAL_ECHOPGM(" encoder raw: ");
//    LOOP_L_N(i, 32) SERIAL_ECHO(0x00000001 & (encoderCount.val >> (31-i)));
//    SERIAL_ECHOLN(" ;");
...
//    SERIAL_ECHO(axis_codes[encoderAxis]);
//    SERIAL_ECHOPGM(" encoder after H ext.: ");
//    LOOP_L_N(i, 32) SERIAL_ECHO(0x00000001 & (encoderCount.val >> (31-i)));
//    SERIAL_ECHOPGM(" - ");
//    SERIAL_ECHO(encoderCount.val);
//    SERIAL_ECHOLN(" ;");
